...
export const generateInvoicePDF = (invoiceData: any) => {
  ...
  doc.text(
    "Nandgola, Patna, Patna, Bihar, 800008",
    marginX,
    (y += lineSpacing + 9),
  );
  ...
  const alignRightSection = (
    ...
    if (isBold) {
      ...
      const imageX =
        valueX - doc.getTextWidth(valueText) - imageSize - imageMarginRight;
  ...
  alignRightSection("Balance Due:", invoiceData.totalAmount || 0, true); // with icon
  ...
  const totalInWords = numberToWordsIndian(totalAmount);

  doc.text(`Total in Words: ${totalInWords} Only`, marginX, leftY);
  leftY += lineSpacing * 2;
  doc.setFont("helvetica", "normal");
